...
- **`not_found`** — DOI.org HEAD (the protocol-level resolver, which covers every DOI registration agency) explicitly could not resolve this DOI. The citation is wrong. (A Crossref miss alone is *not* enough — Crossref doesn't index DataCite/Zenodo DOIs — so only the doi.org resolver is trusted to declare absence.)
...
## How `science-mentor` handles it

In `science-mentor`, the verifier returns a `DoiCheckResult` with one of the three states. The aggregate `citation_validity_rate` is computed as:
...
Always-on:
1. **OpenAlex** — fast, free, useful for indexed publications
2. **Crossref** — authoritative for positive Crossref-indexed hits, but not for absence
...
- If DOI.org HEAD returns `not_found` → result is `not_found`. It is the only absence authority because it resolves across DOI registration agencies.
- Otherwise (all sources returned `error` or non-authoritative `not_found`) → result is `verifier_error`.

This is enforced by `_build_chain()` and the `is_authoritative_for_existence` flag. Crossref deliberately does **not** have that flag: a Crossref miss can be an indexing gap for DataCite/Zenodo/mEDRA DOIs, not proof that the DOI is fabricated.
...
There's one more subtle bit. In the publication-strategy mode of `science-mentor`, when a ref hits `verifier_error`, the verifier attaches `verifier_error_metadata` (last error, sources tried, timestamp) and the Markdown renderer adds a ⚠️ flag next to that citation. The user sees the flag in their browser and can manually re-check.
...
See `scripts/doi_verify_multisource.py` and `tests/test_doi_verify_multisource.py` in [github.com/dalek12310/science-mentor](https://github.com/dalek12310/science-mentor) for the implementation, including the monkeypatched tests that verify each chain branch (found / not_found / error) lands in the right bucket.
